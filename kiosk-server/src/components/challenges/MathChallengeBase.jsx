...
export function makeMathConfigSchema({ description, minDefault = 10, maxDefault = 99 }) {
  return {
...
export default function MathChallengeBase({ config = {}, defaults, generateProblem, challengeType, reward, onComplete, onBack }) {
  ...
  const handleSubmit = useCallback((e) => {
    ...
    if (guess === problem.answer) {
      ...
      if (next >= totalProblems) {
        setDone(true);
        onComplete(challengeType, reward);
      } else {
        setProgress(next);
        setProblem(generateProblem(minNum, maxNum));
        setInput('');
      }
    } else {
      setShake(true);
  ...
  }, [input, problem, progress, reward, onComplete, totalProblems, minNum, maxNum, challengeType, generateProblem]);
